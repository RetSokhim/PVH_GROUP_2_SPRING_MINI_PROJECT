...
public class ExpenseController {
    ...
    public ResponseEntity<?> getAllExpense(
        ...
        try {
            ...
            if (totalExpense == 0) {
                throw new SearchNotFoundException("There are no expenses. Please add some.");
            }
            ...
            ApiResponse<?> apiResponse = new ApiResponse<>("Get all expenses successfully", userExpense, LocalDateTime.now(), 201, HttpStatus.OK);
            return new ResponseEntity<>(apiResponse, HttpStatus.OK);
        } catch (SearchNotFoundException | PageLimitException exception) {
            ApiResponse<?> apiResponse = new ApiResponse<>(exception.getMessage(), null, LocalDateTime.now(), 400, HttpStatus.BAD_REQUEST);
    ...
    public ResponseEntity<?> getExpenseById(@PathVariable UUID expenseId) throws SearchNotFoundException {
        try {
            ...
            ApiResponse<?> apiResponse = new ApiResponse<>("Get expense with ID " + expenseId + " successfully", userExpense, LocalDateTime.now(), 200, HttpStatus.OK);
            return new ResponseEntity<>(apiResponse, HttpStatus.OK);
        } catch (SearchNotFoundException exception) {
            ApiResponse<?> apiResponse = new ApiResponse<>(exception.getMessage(), null, LocalDateTime.now(), 404, HttpStatus.NOT_FOUND);
    ...
    public ResponseEntity<?> deleteExpenseById(@PathVariable UUID expenseId) throws SearchNotFoundException {
        try {
            ...
            ApiResponse<?> apiResponse = new ApiResponse<>("Deleted expense with ID " + expenseId + " successfully", null, LocalDateTime.now(), 200, HttpStatus.OK);
            return new ResponseEntity<>(apiResponse, HttpStatus.OK);
        } catch (SearchNotFoundException exception) {
            ApiResponse<?> apiResponse = new ApiResponse<>(exception.getMessage(), null, LocalDateTime.now(), 404, HttpStatus.NOT_FOUND);
            return new ResponseEntity<>(apiResponse, HttpStatus.NOT_FOUND);
    ...
    public ResponseEntity<?> insertNewExpense(@Valid @RequestBody ExpenseRequestDTO expenseRequestDTO) throws SearchNotFoundException {
        try {
            ...
            ApiResponse<?> apiResponse = new ApiResponse<>("New expense has been added successfully", expenseResponse, LocalDateTime.now(), 200, HttpStatus.CREATED);
    ...
    public ResponseEntity<?> updateExpenseById(@PathVariable UUID expenseId, @Valid @RequestBody ExpenseRequestDTO expenseRequestDTO) throws SearchNotFoundException {
        try {
            Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
            String email = authentication.getName();
            User user = userService.findUserByEmail(email);
            ExpenseResponse userExpense = expenseService.updateExpenseById(expenseId, expenseRequestDTO, user.getUserId());
            ApiResponse<?> apiResponse = new ApiResponse<>("Updated expense with ID " + expenseId + " successfully", userExpense, LocalDateTime.now(), 200, HttpStatus.OK);
            return new ResponseEntity<>(apiResponse, HttpStatus.OK);
        } catch (SearchNotFoundException exception) {
            ApiResponse<?> apiResponse = new ApiResponse<>(exception.getMessage(), null, LocalDateTime.now(), 404, HttpStatus.NOT_FOUND);
            return new ResponseEntity<>(apiResponse, HttpStatus.NOT_FOUND);
    ...
    private boolean isValidOrderBy(String orderBy) {
        return orderBy.equals("expense_id") || orderBy.equals("date") || orderBy.equals("amount");
    }
}
