...
public interface OtpsRepository {
    @Select("""
    INSERT INTO otps_tb(otps_code, issued_at, expiration, user_id) VALUES (#{user.otpsCode},#{user.issuedAt},#{user.expiration},#{user.user});
    """)
    ...
    void insertOtp(@Param("user") OtpsRequest otpsRequest);

    @Select("""
    SELECT * FROM otps_tb WHERE otps_code = #{otpsCode}
    """)
    @Result(property = "otpsId",column = "otps_id")
    @Result(property = "otpsCode",column = "otps_code")
    @Result(property = "issuedAt",column = "issued_at")
    ...
    @Select("""
    SELECT * FROM otps_tb WHERE user_id = #{userId}
    """)
    @Result(property = "otpsId",column = "otps_id")
    @Result(property = "otpsCode",column = "otps_code")
    @Result(property = "issuedAt",column = "issued_at")
    ...
    """)
    @ResultMap("otpsMapping")
    void updateTheCodeAfterResend (@Param("otps") OtpsRequest otpsRequest, Integer userId);
}
